    ...
    <div class="container">
        ...
        if (isset($_SESSION["username"])) {
            ...
            function excluirDadosConta($conexao, $username) {
             ...
             // Função para excluir os dados associados ao usuário
             function excluirOfertasConta($conexao, $username) {
                // Ajuste o nome da coluna para `user_id`
                $sql = "DELETE FROM PedidosOfertas WHERE user_id = :username";
                $stmt = $conexao->prepare($sql);
                $stmt->bindParam(':username', $username, PDO::PARAM_STR);
                return $stmt->execute();
            }
            ...
            try {
                ...
                if (excluirDadosConta($conexao, $username) && excluirConta($conexao, $username) && (excluirOfertasConta($conexao, $username))) {
                    $conexao->commit();
