...
$TituloAtual = $_GET['titulo'];
$DescricaoAtual = $_GET['descricao'];

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Verifique se os campos 'titulo' e 'descricao' estão definidos
    if (isset($_POST['titulo']) && isset($_POST['descricao'])) {

        $id = $_GET['id'];

        $resultado = $novidadeController->editar($_POST['titulo'], $_POST['descricao'], $id);

        if ($resultado) {
            echo "<p>Novidade editada com sucesso!</p>";
            header ('Location: novidade_funcionario.php');
        } else {
            echo "<p>Erro ao editar a novidade. Tente novamente.</p>";
        }
    } else {
...
<head>
    ...
    <link rel="stylesheet" href="../../../style/funcionario/novidade/novidade_editar.css">
...
<body>
    ...
    <section class="section-form">
        <div class="div-form">
            ...
            <form class="form-novidade" method="post">
                <label for="titulo">Titulo:</label>
                <input class="input-form" type="text" id="titulo" name="titulo"  value="<?= htmlspecialchars($TituloAtual) ?>"  required>

                <label for="descricao">Descrição:</label>
                <textarea id="descricao" name="descricao" required><?= htmlspecialchars($DescricaoAtual) ?></textarea>
