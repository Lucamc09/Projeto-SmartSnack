<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $resultado = $novidadeController->excluir($_POST['id']);
    
            if ($resultado) {
                echo "<p>Novidade excluida com sucesso!</p>";
                header("Location: novidade_funcionario.php");
            } else {
                echo "<p>Erro ao excluir a novidade. Tente novamente.</p>";
            }
        }
?>
<!DOCTYPE html>
...
<head>
    ...
    <link rel="stylesheet" href="../../../style/funcionario/novidade/novidade_funcionario.css">
...
?>
<section>
    <span class="span-titulo">Novidade:</span>
    <hr>
</section>

<section>
    <a href="novidade_adicionar.php"><button class="botao-verde">Adicionar</button></a>
</section>  
...
<section class="section-novidade">
    <?php foreach ($items as $item): ?>
        <div class="div-novidade">
            ...
            <a href="novidade_editar.php?id=<?php echo $item['id']; ?>&titulo=<?php echo urlencode($item['titulo']); ?>&descricao=<?php echo urlencode($item['descricao']); ?>" class="botao-branco">Editar</a>
           <!-- Formulário para Excluir -->
            <form action="" method="POST">
                <!-- Passando o ID da novidade via campo oculto -->
                <input type="hidden" name="id" value="<?= $item['id']; ?>">

                <!-- Botão para Excluir -->
                <button class="botao-branco" type="submit">Excluir</button>
            </form>
